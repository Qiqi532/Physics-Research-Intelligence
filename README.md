...
首个个人版本建议部署在一台香港或新加坡的 Linux VPS：PostgreSQL 与 Redis 使用持久卷，Web 与 worker 作为两个独立的常驻进程，Next.js 前放 Nginx/Caddy 等反向代理并启用 HTTPS。2 vCPU、4 GB 内存和 40 GB 磁盘可作为小规模试运行起点，实际容量应按论文量、构建峰值和备份保留期调整。
