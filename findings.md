...
- 现有 `Paper` 级联关系适合集中清理过期非收藏论文，但清理必须在 Today 准备之后执行，失败只记录稳定错误码，不能让 Today 整页不可用。
- 当前每日流水线已具备稳定时区窗口、BullMQ scheduler、采集/分类/解读幂等和预算边界；阶段 9A 应在这些边界上增加 10–15 篇稳定选择和清理，不重写调度器。
- `data/review-corpus/pdfs/` 是人工评审语料，不是个人收藏库；未来本地资产必须使用独立忽略目录、数据库资产元数据、SHA-256 和明确许可决策。
- 自动 PDF 保存只接受明确开放许可；许可证缺失或含糊时失败关闭。手动导入合法个人 PDF 不等于允许上传模型或再分发，模型使用权需要独立可见状态。
- 当前 provider 合约面向分类/解读 completion，没有通用对话和 embedding 能力；阅读助手与 RAG 必须分别设计能力契约，不能把不支持 embedding 的聊天供应商假定为兼容。
- 个人规模 RAG 初始建议复用 PostgreSQL 保存稳定 chunks/embeddings，并在应用侧做有界相似度计算，避免立即引入 pgvector 或新服务；达到实际规模/延迟瓶颈后再用测量结果决定扩展。
- 详细路线图：`docs/superpowers/plans/2026-09-01-local-library-ai-reading-roadmap.md`。下一对话只启动 9A，9B PDF、阶段 10 阅读助手和阶段 11 RAG 分别审查设计。
...
- 动态路由 UUID 必须在 service 进入 Prisma 前校验；routing 也需要显式公开 DTO，不能返回带 `userId` 和 Date 对象的仓储行。
