...
- [ ] 为 `SourceConnector` 定义统一返回类型：记录、来源 URL、许可证、游标、重试建议和可见错误码。
- [ ] 对每个来源先写 fixture 测试：缺 DOI、空摘要、分页游标、429 与临时 5xx 必须产生预期状态。
- [ ] 实现按日期增量采集、请求超时、指数退避和来源级最后成功时间；所有 HTTP 调用设定用户代理和速率限制。
- [ ] 将来源记录写入事实层并执行 DOI 去重；任务失败只标记该来源，不能使其他来源任务失败。
- [ ] 使用录制 fixture 跑连接器集成测试，不在测试中访问真实生产 API。
